...
/turf/simulated/mineral/attackby(var/obj/item/weapon/pickaxe/P as obj, mob/user as mob, params)
	...
	if(istype(P, /obj/item/weapon/pickaxe))
		...
		if(last_act+P.digspeed > world.time)//prevents message spam
			return
		last_act = world.time
		to_chat(user, "<span class='notice'>You start using your [P] to remove the solid asteroid.</span>")
		P.playDigSound()

		switch(do_after_stat(user, delay = 70, needhand = 1, target = src, progress = 1, action_name = "dig away rock", auto_emote = 1, stat_used = 3, minimum = 0, maximum = 8, maxed_delay = 40, progressive_failure = 0, minimum_probability = 70, help_able = 0, help_ratio = 1, stamina_use = 1, stamina_used = 5, progressive_stamina = 1, attempt_cost = 5, stamina_use_fail = 1))
			if(1)
