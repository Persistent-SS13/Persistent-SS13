...
/obj/effect/hotspot
	...
	var/volume = 125
	var/temperature = FIRE_MINIMUM_TEMPERATURE_TO_EXIST + 25
	var/just_spawned = 1
	var/bypassing = 0
...
/obj/effect/hotspot/process()
	if(just_spawned)
		...
		if(!istype(location) || !(location.air))	return 0
			...
			if(location.air.oxygen < 1)
				var/datum/gas_mixture/ignition = location.remove_air(0.9*location.air.total_moles())
				ignition.temperature = rand(10000)
				location.air.merge(ignition)
			return 0
	...
	if(!(location.air) || location.air.toxins < 0.5)
		qdel(src)
		return
	...
	if(istype(loc, /turf/simulated))
		...
		if(T.to_be_destroyed)
			...
			if(prob(chance_of_deletion))
				T.ChangeTurf(T.replacement_type)
			else
				T.to_be_destroyed = 0
