	...
	if(!istype(M, /mob/living/carbon/human))
		...
		for(var/mob/O in viewers(messagesource, null))
			if(attack_verb.len)
				...
				// PERSISTNANT EDIT! generic attacks need to reflect damage type (this file could probobly be used to create anti-grief and anti-helpintent attack. measures)
				if(damtype == "fire")
					O.show_message("<span class='danger'>[M] has been burned with [src][showname] </span>", 1)
				else if(damtype == "brute")
					if(sharp)
						O.show_message("<span class='danger'>[M] has been stabbed with [src][showname] </span>", 1)
					else
