...
/datum/controller/process/shuttle
	...
	var/emergencyCallTime = 3000	//time taken for emergency shuttle to reach the station when called (in deciseconds) 5 MINS
	var/emergencyDockTime = 3000	//time taken for emergency shuttle to leave again once it has docked (in deciseconds) 5 MINS
	var/emergencyEscapeTime = 920	//time taken for emergency shuttle to reach a safe distance after leaving station (in deciseconds) 1 MINS
...
//		supply_packs["[P.type]"] = P
	for(var/typepath in subtypesof(/datum/supply_item))
		var/datum/supply_item/P = new typepath()
		if(P.name == "HEADER") continue		// To filter out group headers
		supply_packs["[P.type]"] = P
	initial_move()
...
//		return
	...
	switch(emergency.mode)
		if(SHUTTLE_RECALL)
			to_chat(user, "The departure shuttle may not be called while returning to Centcom.")
			return
		if(SHUTTLE_CALL)
			to_chat(user, "The departure shuttle is already on its way.")
			return
		if(SHUTTLE_DOCKED)
			to_chat(user, "The departure shuttle is already here.")
			return
		if(SHUTTLE_ESCAPE)
			to_chat(user, "The departure shuttle is moving away to a safe distance.")
			return
		if(SHUTTLE_STRANDED)
			to_chat(user, "The departure shuttle has been disabled by Centcom.")
			return
		if(SHUTTLE_BOARDING)
			to_chat(user, "The departure shuttle is already on its way.")
			return	
		...
		if(SHUTTLE_TRANSITTO)
			...
			return
		if(SHUTTLE_ARRIVED)
			to_chat(user, "The departure shuttle has just arrived.")
			return
		if(SHUTTLE_RETURNING)
			to_chat(user, "The departure shuttle has just left, wait until it reaches Centcom and refuels.")
			return
		if(SHUTTLE_DELAY)
			to_chat(user, "The departure shuttle is in transit.")
			return
		if(SHUTTLE_DEPDELAY)
			to_chat(user, "The departure shuttle is in transit.")
			return
		...
		if(SHUTTLE_DELAY3)
			...
			return
		if(SHUTTLE_ENDING)
			to_chat(user, "It's too late now, the deployment is over.")
			return
		if(SHUTTLE_ENDGAME)
			to_chat(user, "It's too late now, the deployment is over.")
			return	
...
#define SHUTTLE_BOARDING 7
#define SHUTTLE_WAITING 8
#define SHUTTLE_TRANSITTO 9
#define SHUTTLE_ARRIVED 10
#define SHUTTLE_RETURNING 11
#define SHUTTLE_RETURNED 12
#define SHUTTLE_DELAY 13
#define SHUTTLE_DEPDELAY 14
#define SHUTTLE_DELAY2 15
#define SHUTTLE_TRAVELING 16
#define SHUTTLE_CALLED 17
#define SHUTTLE_DELAY3 18
#define SHUTTLE_ENDING 19
...
/datum/controller/process/shuttle/proc/moveShuttle(shuttleId, dockId, timed, var/firstmove = 0)
	var/obj/docking_port/mobile/M = getShuttle(shuttleId)
	if(!M)
		return 1
	if(timed)
		...
		if(M.dock(getDock(dockId), firstmove))
			return 2
	return 0	//dock successful
	...
	for(var/obj/docking_port/mobile/M in mobile)
		...
		for(var/obj/docking_port/stationary/S in stationary)
			if(!is_station_level(S.z) && findtext(S.id, M.id))
				...
				S.dwidth = M.dwidth
				S.dheight = M.dheight
		moveShuttle(M.id, "[M.roundstart_move]", 0, 1)
