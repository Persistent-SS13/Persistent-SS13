...
proc/get_id_photo_new(var/mob/living/Hu) // PERSISTANT AND MARKED FOR CHANGE
	...
	if(istype(H, /obj))
		obbie = H
		obbie.deleting = 1
	else if(istype(H, /mob))
		mobbie = H
		mobbie.deleting = 1
	if(obbie)
		var/icon/I = new(obbie.icon, obbie.icon_state,  dir = SOUTH)
		for(var/A in obbie.underlays) 
			I.Blend(icon(A:icon,A:icon_state,SOUTH,1),ICON_OVERLAY)
		for(var/A in obbie.overlays)
			I.Blend(icon(A:icon,A:icon_state,SOUTH,1),ICON_OVERLAY)		
		qdel(H)
		return I
	if(mobbie)
		var/icon/I = new(mobbie.icon, obbie.icon_state,  dir = SOUTH)
		for(var/A in mobbie.underlays) 
			I.Blend(icon(A:icon,A:icon_state,SOUTH,1),ICON_OVERLAY)
		for(var/A in mobbie.overlays)
			I.Blend(icon(A:icon,A:icon_state,SOUTH,1),ICON_OVERLAY)		
		qdel(H)	
		return I
...
proc/get_id_photo(var/mob/living/Hu) // PERSISTANT AND MARKED FOR CHANGE
	...
	if(obbie)
		...
		return I
	if(mobbie)
		var/icon/I = new(mobbie.icon, mobbie.icon_state,  dir = SOUTH)
		for(var/A in mobbie.underlays) 
			I.Blend(icon(A:icon,A:icon_state,SOUTH,1),ICON_OVERLAY)
		for(var/A in mobbie.overlays)
			I.Blend(icon(A:icon,A:icon_state,SOUTH,1),ICON_OVERLAY)		
...
proc/get_id_photo_old(var/mob/living/carbon/human/H) // TRASH IT
