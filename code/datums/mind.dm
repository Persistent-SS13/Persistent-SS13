/datum/mind
	...
	var/cert_title = ""
	var/default_slot = 0  
	var/has_been_promoted = 0
	var/initial_department = SUPPORT
	var/list/ranks // for promotions format ranks(MEDICAL) = (1 - 4)
	var/datum/faction/faction
	var/faction_uid = ""
	var/codename
	var/energy_creds = 0 // this var is just used to save and load the credits into the ATM account
	var/account = list() // saved money
	var/on_assignment = 0
	var/obj/item/weapon/card/id/spawned_id = null
	var/allocated = 0 // ALLOWS non-heads to be allocated department funds to spend.
	var/stat_Grit = 0
...
/datum/mind/proc/show_memory(mob/recipient)
	...
	if(istype(assigned_job, /datum/cert/))
		output += "<HR><B>Department Quota:</B><UL>"
		var/obj_count = 1
		var/datum/department/department = get_department_datum(assigned_job.department_flag)
		for(var/datum/job_objective/department/job in department.objectives)
			output += "<LI><B>Task #[obj_count]</B>: [job.get_description()]</LI>"
			obj_count++
