/atom
	...
	var/tmp/throwpass = 0
	var/tmp/germ_level = GERM_LEVEL_AMBIENT // The higher the germ level, the more germ on the atom.
	var/tmp/simulated = 1 //filter for actions - used by lighting overlays
	var/tmp/atom_say_verb = "says"
	var/tmp/dont_save = 0 // For atoms that are temporary by necessity - like lighting overlays
	...
	var/tmp/explosion_block = 0

	//Detective Work, used for the duplicate data points kept in the scanners
	var/tmp/list/original_atom

	var/tmp/allow_spin = 1 //Set this to 1 for a _target_ that is being thrown at; if an atom has this set to 1 then atoms thrown AT it will not spin; currently used for the singularity. -Fox
