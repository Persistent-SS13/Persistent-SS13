...
/obj/var/list/req_personal = list() // add real name of people who should be able to  open
//returns 1 if this mob has sufficient access to use this object
/obj/proc/allowed(mob/M)
	//check if we don't require any access at all
	if(req_personal.len)
		var/approved = 0
		for(var/obj/item/device/pda/temp in M.contents)
			if(temp.id && temp.id.assigned_mind && temp.id.assigned_mind.name in req_personal)
				approved = 1
				break
		if(!approved)
			for(var/obj/item/weapon/card/id/tempp in M.contents)
				if(tempp && tempp.assigned_mind && tempp.assigned_mind.name in req_personal)
					approved = 1
					break
		if(approved)
			return 1
