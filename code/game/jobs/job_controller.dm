...
/datum/controller/occupations
	...
	proc/SetupCertifications(var/list/faction = list("Station"))
		certifications = list()
		var/list/all_jobs = subtypesof(/datum/cert)
		if(!all_jobs.len)
			to_chat(world, "\red \b Error setting up jobs, no job datums found")
			return 0
		for(var/J in all_jobs)
			var/datum/cert/job = new J()
			if(!job)	continue
			if(!job.faction in faction)	continue
			certifications += job		
		return 1
	...
	proc/GetCert(var/rank)
		if(!rank)	return null
		for(var/datum/cert/C in certifications)

			if(!C)	continue
			if(C.uid == rank)	return C
			if(C.title == rank) return C
		return null

	proc/GetPlayerAltTitle(mob/new_player/player, rank)
		return player.client.prefs.GetPlayerAltTitle(GetJob(rank))

	proc/AssignRole(var/mob/new_player/player, var/rank, var/latejoin = 0) // ASSIGN ROLE
		...
		if(player && player.mind && rank)
			...
			if((job.current_positions < position_limit) || position_limit == -1)
				...
				player.mind.role_alt_title = player.client.prefs.cert_title
	...
	proc/AssignPersistantRole(var/mob/new_player/player, var/rank, var/latejoin = 0) // ASSIGN ROLE APR APR APR
		Debug("Running AR, Player: [player], Rank: [rank], LJ: [latejoin]")
		if(player && player.mind && rank)
			...
			player.mind.faction_uid = player.client.prefs.faction
			// JOB OBJECTIVES OH SHIT
			player.mind.job_objectives.Cut()
			for(var/objectiveType in job.required_objectives)
				new objectiveType(player.mind)
 ...
 **/
	...
	proc/DividePersistant() // DOP DOP DOP
		//Setup new player list and get the jobs list
		Debug("Running DOP")
		SetupCertifications()
	...
	//	Debug("DO, Running Standard Check")
		...
		for(var/mob/new_player/player in unassigned)
			for (var/datum/cert/G in certifications)
				if(player.client.prefs.primary_cert == G.uid)
					AssignPersistantRole(player, player.client.prefs.primary_cert)
					break		
	...
	//	message_admins("[rank]")
		...
		if(!H.mind.ranks)
			...
			for(var/a in ranks)
				...
				H.mind.ranks[stringrank] = 1
		if(!joined_late)
			var/obj/S = null
			var/obj/holder = null
			if(istype(H.loc, /obj))
				holder = H.loc
				if(istype(holder.loc, /obj))
					holder = holder.loc
			if(holder)
				for(var/obj/effect/landmark/start/sloc in landmarks_list)
					if(sloc.name != job.title)	continue
					if(locate(/mob/living) in sloc.loc)	continue
					S = sloc
					break
				if(!S)
					S = locate("start*[job.title]") // use old stype
				if(istype(S, /obj/effect/landmark/start) && istype(S.loc, /turf))
					holder.loc = S.loc
				else
					holder.loc = pick(latejoin)			
		...
		if(H.mind)
			...
			display_name = get_default_title(H.mind.ranks[to_strings(job.department_flag)], job)
	//		switch(rank)
	//			if("Cyborg")
	//				return H.Robotize()
	//			if("AI")
	//				return H
		...
		to_chat(H, "<B>You were assigned to [alt_title ? alt_title : display_name].</B>")
		if (job.uid == "intern")
			to_chat(H, "<b>As an [alt_title ? alt_title : display_name] you answer directly to [job.supervisors]. Follow the orders of senior staff and work to be assigned to a department.</b>")
		else
			to_chat(H, "<b>As an [alt_title ? alt_title : display_name] you answer directly to [job.supervisors]. Work with your colleagues to complete your quota and get paid!</b>")
		...
		var/datum/department/department = get_department_datum(job.department_flag)	
		if(department != "ERROR!")
			var/objcount = 1
			for(var/datum/job_objective/department/jeb in department.objectives)
				to_chat(H, "<b><LI><B>Task #[objcount]</B>: [jeb.get_description()]</LI>")
				objcount += 1
		...
		if(istype(H, /mob/living/carbon/human))
			...
			Ha.species.equip(H)
			Ha.sec_hud_set_ID()
			Ha.sec_hud_set_implants()
		if(istype(H, /mob/living/simple_animal/spiderbot))
			H.icon_state = "spiderbot-chassis-mmi"
		return H

			
		
	proc/spawnIdNew(var/mob/living/carbon/human/H, var/rank, var/list/ranklist, var/title)
		if(!H || !H.mind)
			return 0
		...
		if(C)
			...
			C.rank = job
			C.assignment = title ? title : get_default_title(H.mind.ranks[to_strings(job.department_flag)], job)
