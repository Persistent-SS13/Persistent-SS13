...
/obj/machinery/clonepod/proc/growclone(var/datum/dna2/record/R, var/function = 1, var/brain_insert = 0)
	...
	for(var/obj/item/organ/internal/brain/B in H.internal_organs)
		if(istype(B))
			B.remove(H, 1)
			qdel(B)
	if(!R.dna.real_name)	//to prevent null names
		if(held_brain && held_brain.brainmob)
			R.dna.real_name = held_brain.brainmob.mind.name			
		else
			R.dna.real_name = "CLONE"
	...
	//	held_brain.brainmob.mind.transfer_to(H)
		...
		to_chat(H, "<span class='notice'><b>Your consciousness fades in and out as a body generates around your brain.</b><br></span>")// <i>So this is what cloning feels like?</i> implement achievment or first time experience system
...
/obj/machinery/clonepod/proc/growcloneold(var/datum/dna2/record/R, var/brain_insert = 0)
	if(mess || attempting || panel_open || stat & (NOPOWER|BROKEN))
		return 0
...
/obj/machinery/clonepod/attackby(obj/item/weapon/W as obj, mob/user as mob, params)
	if(istype(W, /obj/item/organ/internal/brain))
		if(held_brain.brainmob || occupant)
			to_chat(user, "There's already an occupant in the pod.")
			return
		var/obj/item/organ/internal/brain/brain = W
		if(brain.brainmob)
			held_brain.attackby(W, user)
			...
			to_chat(user, "<span class='notice'>This brain is inactive.</span>")
			return
...
/obj/machinery/clonepod/proc/go_out(user)
	...
	if(held_brain.brainmob)
		if(held_brain.brainmob.client)
			held_brain.brainmob.client.eye = held_brain.brainmob
			held_brain.brainmob.client.perspective = MOB_PERSPECTIVE
		held_brain.dropbrain(get_turf(src))
		eject_wait = 0
		update_icon()
		return
