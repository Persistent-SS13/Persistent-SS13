...
/obj/machinery/computer/communications/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null)
	...
	switch(shuttle_master.emergency.mode)
		if(SHUTTLE_IDLE, SHUTTLE_RECALL, SHUTTLE_RETURNED)
...
/proc/call_shuttle_proc(var/mob/user, var/reason)
	...
	if(shuttle_master.emergency.mode != SHUTTLE_IDLE && shuttle_master.emergency.mode != SHUTTLE_RETURNED && shuttle_master.emergency.mode != SHUTTLE_IDLE && shuttle_master.emergency.mode != SHUTTLE_CALLED)
