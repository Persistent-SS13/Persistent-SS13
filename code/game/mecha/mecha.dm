...
/obj/mecha
	...
	var/obj/item/device/mmi/active_mmi
	var/brain_op = 0
...
/obj/mecha/proc/dyndomove(direction)
	...
	if(move_result)
		...
		if(mecha_do_after(step_in + step_delay))
			step_delay = 0
			can_move = 1
		return 1
	return 0
...
/obj/mecha/proc/mechstep(direction)
	var/turf/T = src.loc
	if(occupant.pulling)
		if(!occupant.pulling.get_move_able())
			occupant.visible_message("<span class='info'>[src] struggles until its hyrdaulics whine but it still cant move [occupant.pulling].</span>")
			to_chat(occupant, "You do not have enough grit to move [occupant.pulling]. stat up, get a better mech or get someone to help pull on the other side.")
			if(!occupant.Adjacent(occupant.pulling))
				occupant.stop_pulling()
			step_delay = 25//set move delay
			return
		else
			occupant.pulling.affect_stamina()
	var/result = step(src,direction)
	if(result && stepsound)
		playsound(src,stepsound,40,1)
	if(result)
		if(occupant.pulling)
			var/diag = get_dir(src, occupant.pulling)
			if((diag - 1) & diag)
			else
				diag = null
			if((get_dist(T, occupant.pulling) >= 1 || diag))
				if(isliving(occupant.pulling))
					var/mob/living/M = occupant.pulling
					var/ok = 1
						...
						if(prob(75))
							...
							if(istype(G, /obj/item/weapon/grab))
								for(var/mob/O in viewers(M, null))
									O.show_message(text("\red [] has been pulled from []'s grip by the []", G.affecting, G.assailant, src), 1)
								//G = null
								qdel(G)
						else
							ok = 0
					...
					if(ok)
						...
						occupant.pulling.Move(T, get_dir(occupant.pulling, T))
						if(M)
							M.start_pulling(t)
				else
					if(occupant.pulling)
						occupant.pulling.Move(T, get_dir(occupant.pulling, T))
				step_delay = occupant.pulling.calculate_movedelay()
	
	return result

/obj/mecha/proc/mechsteprand()
	var/dir = pick(1,2,4,8)
	return mechstep(dir)
...
/obj/mecha/proc/mmi_move_inside(var/obj/item/device/mmi/mmi_as_oc as obj,mob/user as mob)
	...
	//	return 0
		...
		to_chat(user, "DNA authentication failed! Please contact the owner of this mecha.")
		return 0
...
/obj/mecha/proc/mmi_moved_inside(var/obj/item/device/mmi/mmi_as_oc as obj,mob/user as mob, var/override = 0)
	if(override)
		var/mob/brainmob = mmi_as_oc.brainmob
		brainmob.reset_view(src)
		occupant = brainmob
		active_mmi = mmi_as_oc
		brain_op = 1
	...
	*/
		...
		mmi_as_oc.loc = src
		mmi_as_oc.mecha = src
	//	MMIS SHOULD STILL BE ABLE TO EJECT!!!
