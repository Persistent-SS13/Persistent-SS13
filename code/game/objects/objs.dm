/obj
	...
	var/tmp/origin_tech = null	//Used by R&D to determine what research bonuses it grants.
	var/tmp/reliability = 100	//Used by SOME devices to determine how reliable they are.
	var/tmp/crit_fail = 0
	var/tmp/unacidable = 0 //universal "unacidabliness" var, here so you can use it in any obj.
	animate_movement = 2
	var/tmp/throwforce = 1
	var/tmp/list/attack_verb = list() //Used in attackby() to say how something was attacked "[x] has been [z.attack_verb] by [y] with [z]"
	...
	var/tmp/savedowner = "" // PERSISTANT this is something that should be saved to determine item ownership
	var/tmp/deleting = 0 // PERSISTANT used to delete the brain
	var/tmp/save_obj = 1 // THIS DETERMINES WHETHER THE ITEM SAVES OR NOT
	var/tmp/datum/gas_mixture/air_contents = null
	
	
	var/tmp/damtype = "brute"
	var/tmp/force = 0

	var/tmp/Mtoollink = 0 // variable to decide if an object should show the multitool menu linking menu, not all objects use it

	var/tmp/burn_state = FIRE_PROOF // LAVA_PROOF | FIRE_PROOF | FLAMMABLE | ON_FIRE
	var/tmp/burntime = 10 //How long it takes to burn to ashes, in seconds
	var/tmp/burn_world_time //What world time the object will burn up completely
	var/tmp/being_shocked = 0

	var/tmp/on_blueprints = FALSE //Are we visible on the station blueprints at roundstart?
	var/tmp/force_blueprints = FALSE //forces the obj to be on the blueprints, regardless of when it was created.
	map_storage_saved_vars = "density;icon_state;dir;name;pixel_x;pixel_y;req_access_txt;req_personal"
