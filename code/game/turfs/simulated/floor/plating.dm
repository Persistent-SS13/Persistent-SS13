/turf/simulated/floor/plating/bulkhead
	name = "bulkhead"
	heat_capacity = INFINITY
	icon_state = "alienplating"
/turf/simulated/floor/plating/bulkhead/burn_tile()
	return 0
	
/turf/simulated/floor/plating/bulkhead/attackby(obj/item/C, mob/user, params)
	if(istype(C, /obj/item/stack/rods))
		var/obj/item/stack/rods/R = C
		var/obj/structure/lattice/L = locate(/obj/structure/lattice, src)
		if(L)
			to_chat(user, "<span class='warning'>There is already a lattice on this bulkhead!</span>")
			return
		if(R.amount)
			to_chat(user, "<span class='notice'>Constructing support lattice...</span>")
			switch(do_after_stat(user, 70, needhand = 1, target = src, progress = 1,
			action_name = "construct a lattice over the bulkhead",
			auto_emote = 1, stat_used = 3, minimum = 2, maximum = 8, maxed_delay = 30,
			progressive_failure = 1, minimum_probability = 70, help_able = 1, help_ratio = 1,
			stamina_use = 2, stamina_used = 5, progressive_stamina = 1, attempt_cost = 5, stamina_use_fail = 1.5))
				if(1)
					if(R.use(1))
						to_chat(user, "<span class='warning'>You finish the lattice.</span>")
						playsound(src, 'sound/weapons/Genhit.ogg', 50, 1)
						new /obj/structure/lattice(src)
					else
						to_chat(user, "<span class='warning'>You need one rod to build a lattice.</span>")
				if(2)
					to_chat(user, "<span class='warning'>You fail to construct the lattice properly.</span>")
		else
			to_chat(user, "<span class='warning'>You need one rod to build a lattice.</span>")
		return
	...
	if(istype(C, /obj/item/stack/tile/plasteel))
		...
		if(L)
			...
			if(S.amount)
				to_chat(user, "<span class='notice'>Constructing plating...</span>")
				switch(do_after_stat(user, 70, needhand = 1, target = src, progress = 1,
				action_name = "construct plating over the bulkhead",
				auto_emote = 1, stat_used = 3, minimum = 2, maximum = 8, maxed_delay = 30,
				progressive_failure = 1, minimum_probability = 70, help_able = 1, help_ratio = 1,
				stamina_use = 2, stamina_used = 5, progressive_stamina = 1, attempt_cost = 5, stamina_use_fail = 1.5))
					if(1)
						if(S.use(1))
							qdel(L)
							playsound(src, 'sound/weapons/Genhit.ogg', 50, 1)
							to_chat(user, "<span class='notice'>You build the plating.</span>")
							ChangeTurf(/turf/simulated/floor/plating)
						else
							to_chat(user, "<span class='warning'>You need one plasteel sheet to build the plating!</span>")
					if(2)
						to_chat(user, "<span class='warning'>You fail to construct the plating properly.</span>")
			else	
				to_chat(user, "<span class='warning'>You need one plasteel sheet to build the plating!</span>")
		else
