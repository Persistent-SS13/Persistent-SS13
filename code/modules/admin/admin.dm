...
/datum/admins/proc/saveworld()
	set category = "Server"
	set desc="SAAAVE!"
	set name="Save Area (EXPERIMENTAL!)"
	...
	if(!ticker)
		alert("Unable to start the game as it is not set up.")
		return
	var/chooseable = list("Cargo", "Security", "Engineering", "Medical", "Science", "Bridge", "Hallways", "Maintenence")
		
	var/chosen = input("Which Department do you want to save?") as null|anything in chooseable
	if(!chosen)
		return
	spawn(0)
		ticker.saveworld(chosen)

/datum/admins/proc/loadworld()
	set category = "Server"
	set desc="SAAAVE!"
	set name="Load Area (EXPERIMENTAL!)"
	...
	var/chosen = input("Which Department do you want to load?") as null|anything in chooseable
	if(!chosen)
		return
	ticker.loadworld(chosen)
	
/datum/admins/proc/savechar()
	set category = "Server"
	set desc="SAAAVE!"
	set name="Save Your Character (TEST)"
	map_storage = new()
	if(!check_rights(R_SERVER))
		return
		
	spawn(0)
		map_storage.Save_Char(null, usr.mind, null, 1)
/datum/admins/proc/loadchar()
	set category = "Server"
	set desc="SAAAVE!"
	set name="Load Your Character (TEST)"
	map_storage = new()
	if(!check_rights(R_SERVER))
		return
