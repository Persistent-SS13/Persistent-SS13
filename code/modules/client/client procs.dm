...
//		to_chat(src, "ASSET JOB [href_list["asset_cache_confirm_arrival"]] ARRIVED.")
	...
	switch(href_list["_src_"])
		...
		if("savevars")	return saved_vars_Topic(href, href_list, hsrc)
...
/client/proc/handle_null_ambient_music()
	qdel(current_song)
	current_song = new()
	spawn _MusicEngine(null, src, time = 20)
/client/proc/handle_ambient_music(var/datum/music_file/music_file)
	if(!mob || !music_file)
		spawn _MusicEngine(null, src, channel=MUSIC_CHANNEL_1, instant = 1)
		spawn _MusicEngine(null, src, channel=MUSIC_CHANNEL_2, instant = 1)
	else
		spawn _MusicEngine(music_file.sound_path, src, time = 20)
		qdel(current_song)
		current_song = new()
		var/datum/curr = current_song
		spawn((music_file.length - 20))
			if(curr != current_song) return 0
			spawn(0)
				if(mob && mob.lastarea)
					if(mob.lastarea.areamusic.len)
						if(mob.lastarea.areamusic.len > 1)
							for(var/datum/music_file/music in mob.lastarea.areamusic)
								if(music.type == music_file.type) continue
								handle_ambient_music(music)
								break
						else
							for(var/datum/music_file/music in mob.lastarea.areamusic)
								handle_ambient_music(music)
