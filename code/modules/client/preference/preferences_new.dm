...
/datum/preferences/proc/CharacterCreateProc(mob/user)
	if(!user || !user.client)
		return
	update_preview_icon()
 ...
 // Character Settings
	...
	if(real_name)
		...
		dat += "<a href='?_src_=prefs;preference=name;task=input'><b>Choose a name<b></a>"	
		dat += "<a href='?_src_=prefs;preference=name;task=random'>(Randomize)</a>"
...
//	dat += "<b>Secondary Language:</b> <a href='?_src_=prefs;preference=language;task=input'>[language]</a><br>"
 // dat += "<b>Nanotrasen Relation:</b> <a href ='?_src_=prefs;preference=nt_relation;task=input'>[nanotrasen_relation]</a><br>"
	...
	dat += "<b>Random Appearance:</b> <a href='?_src_=prefs;preference=all;task=random'>(&reg;)</a><br>"
	...
	dat += "<h2>Choose Starting Role</h2>"
	dat += "<a href='?_src_=prefs;preference=cert;task=select'>Select Starting Certification</a><br>"
	dat += "<b>[get_cert_title()]</b>"
...
//		dat += "<a href='?_src_=prefs;preference=load'>Undo</a> - "
		dat += "<a href=\"byond://?src=\ref[user];preference=confirmchar\">Confirm Creation</a> - "

	dat += "<a href=\"byond://?src=\ref[user];preference=cancelchar\">Cancel Creation</a>"
	dat += "</center>"
//	dat += "<a href=\"byond://?src=\ref[user];preference=open_load_dialog\">Load slot</a> - "

	var/datum/browser/popup = new(user, "charactersetup", "<div align='center'>Create New Character</div>", 820, 640)
	popup.set_content(dat)
	popup.open(0)
		...
		if("input")
			switch(href_list["preference"])
				...
				if("species")
					...
					var/list/new_species = list("Human", "Grey", "Skrell", "Unathi", "Diona", "Kidan")
