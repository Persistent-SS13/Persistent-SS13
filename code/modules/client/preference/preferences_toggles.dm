...
/client/verb/Toggle_Music() // Peristant edit!!  Ambient music has been moved off ambient sounds.. we'll see how THAT goes...
	set name = "Hear/Silence Ambient Music"
	set category = "Preferences"
	set desc = "Toggles hearing ambient music"
	prefs.sound ^= SOUND_BUZZ
	prefs.save_preferences(src)
	if(prefs.sound & SOUND_BUZZ)
		to_chat(src, "You will now hear ambient music.")
	else
		to_chat(src, "You will no longer hear ambient music.")
		src << sound(null, repeat = 0, wait = 0, volume = 0, channel = 3)
	feedback_add_details("admin_verb","TMusic") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
