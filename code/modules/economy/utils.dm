...
/datum/money_account/proc/hold(var/transaction_amount)	
...
/datum/money_account/proc/chargehold(var/transaction_amount,var/datum/money_account/dest,var/transaction_purpose, var/terminal_name="", var/terminal_id=0, var/dest_name = "UNKNOWN")
	if(suspended)
		to_chat(usr, "<span class='warning'>Unable to access source account: account suspended.</span>")
		return 0
	
	if(transaction_amount <= holding)
		//transfer the money
		holding -= transaction_amount
		if(dest)
		...
		to_chat(usr, "<span class='warning'>Held funds have been returned. Transaction Cancelled.</span>")
		return 0
