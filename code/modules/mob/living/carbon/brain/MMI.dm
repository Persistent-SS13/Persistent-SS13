...
/obj/item/device/mmi
	...
	desc = "Brains can be inserted into this mount, allowing them to communicate and even access and control certain electronics."
...
/obj/item/device/mmi/after_load()
	if(!held_brain)
		for(var/obj/item/organ/internal/brain/B in contents)
			held_brain = B
			break
	..()
...
/obj/item/device/mmi/proc/click_action(atom/target, mob/user, params)
	...
	if(istype(target, /obj/machinery/computer/cloning))
		var/obj/machinery/computer/cloning/blam = target
		return blam.attack_hand(user)
	return
...
/obj/item/device/mmi/proc/dropbrain(var/dropspot)
