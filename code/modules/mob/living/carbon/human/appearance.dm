...
/mob/living/carbon/human/proc/change_gender(var/gender, var/update_dna = 1, var/formality = 0)
	if(!formality)
		...
		if(current_fhair && current_fhair.gender != NEUTER && current_fhair.gender != src.gender)
			reset_facial_hair()
