...
/mob/living/carbon/human/get_weight()	
	return species.get_weight()
/mob/living/carbon/human/get_stat(var/stat)
	if(!mind || !species)
		return 0
	var/initial = species.get_stat(stat)
	if(istype(loc, /obj/mecha))
		var/obj/mecha/mech = loc
		initial = mech.get_stat(stat)
	switch(stat)
		if(STAT_GRIT)
			return mind.stat_Grit + initial
		if(STAT_FORTITUDE)
			return mind.stat_Fortitude + initial
		if(STAT_REFLEX)
			return mind.stat_Reflex + initial
		if(STAT_CREATIVITY)
			return mind.stat_Creativity + initial
		if(STAT_FOCUS)
			return mind.stat_Focus + initial
	return 0	
...
/mob/living/carbon/human/attack_animal(mob/living/simple_animal/M as mob)
	if(M.melee_damage_upper == 0)
		...
		var/shar = 0
		var/dam_type = M.melee_damage_type
		if(M.melee_damage_type == "cut")
			shar = 1
			dam_type = "brute"
		apply_damage(damage, dam_type, affecting, armor, sharp = shar, edge = shar)
		updatehealth()
