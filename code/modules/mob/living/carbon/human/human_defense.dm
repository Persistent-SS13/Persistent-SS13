...
/mob/living/carbon/human/proc/attacked_by(var/obj/item/I, var/mob/living/user, var/def_zone)
	...
	if(istype(user, /mob/living/carbon/human) || istype(user, /mob/living/silicon/robot))
		if(user == src)
			if(user.a_intent != "harm")
				to_chat(user, "You cant find any way to use [I.name]")
				return
		else if(user.a_intent == "help")
			to_chat(user, "You show the [I.name] to [src].")
			if(I in src.seen)
				to_chat(src, "[user] shows you the [I.name] again.")
			else
				to_chat(src, "[user] shows you a [I.name].")
				to_chat(src, "[I.desc]")
				src.seen |= I
			return
	if((istype(I, /obj/item/weapon/kitchen/knife/butcher/meatcleaver) || istype(I, /obj/item/weapon/twohanded/chainsaw)) && src.stat == DEAD && user.a_intent == I_HARM)
		...
		if(!src.meatleft)
			...
			qdel(src)
	var/passed = user.zone_sel.selecting
	if(user.zone_sel.selecting == "eyes" || user.zone_sel.selecting == "mouth")
		passed = "head"
	var/obj/item/organ/external/affecting = ran_zone(get_organ(passed))
	...
	if(! I.discrete)
		...
		//	visible_message("<span class='danger'>[src] has been attacked in the [hit_area] with [I.name] by [user]!</span>") PERSISTANT EDIT! ATTACKS SHOULD ALWAYS SHOW DAMAGE TYPE
			if(I.damtype == "fire")
				visible_message("<span class='danger'>[src] has been burned on the [hit_area] with [I.name] by [user] </span>")
			else if(I.damtype == "brute")
				if(I.sharp)
					visible_message("<span class='danger'>[src] has been stabbed in the [hit_area] with [I.name][user] </span>")
				else
					visible_message("<span class='danger'>[src] has been attacked in the [hit_area] with [I.name][user] </span>")
			else
				visible_message("<span class='danger'>[src] has been attacked with [I.name][user] </span>",)
	var/armor = run_armor_check(affecting, "melee", "Your armor has protected your [hit_area].", "Your armor partially protects you", armour_penetration = I.armour_penetration) //Your armor has softened hit to your [hit_area].
...
/mob/living/carbon/human/hitby(atom/movable/AM as mob|obj,var/speed = 5)
	if(istype(AM, /obj/item))
		...
		var/zone = ran_zone_old("chest", 65)
		var/dtype = BRUTE
