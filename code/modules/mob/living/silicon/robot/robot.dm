...
//Hud stuff
	...
	var/obj/screen/inv4 = null // ID SLOT
	var/obj/screen/cells = null // permanent cell HUD
	//
...
//3 Modules can be activated at any one time.
	...
	var/obj/item/device/pda/borg/module_state_4 = null // THIS IS THE ID SLOT
...
// CAMERAS ARE NOT PERSISTANT!!!!
	...
	scanner = new(src)
	scanner.Grant(src)
	module_state_4 = new /obj/item/device/pda/borg(src)
...
/mob/living/silicon/robot/get_access()
	if(!src.mind) // uncontrolled robots always have access
		return IGNORE_ACCESS
	return src.module_state_4.GetAccess()
...
/mob/living/silicon/robot/proc/pick_module_new() // used
	if(module)
	...
	if(installed_module)
		...
		to_chat(src, "No module detected! Talk to a roboticist about getting a module installed.")
		return 0


/mob/living/silicon/robot/proc/pick_module() // not used
	if(module)
		return
...
/mob/living/silicon/robot/Bump(atom/movable/AM, yes)
	if(!(yes) || now_pushing || buckled || !Adjacent(AM))
		return 0
	now_pushing = 1
	if(ismob(AM))
		var/mob/tmob = AM
...
/mob/living/silicon/robot/attackby(obj/item/weapon/W as obj, mob/user as mob, params)
	...
	if(istype(W, /obj/item/weapon/weldingtool) && user.a_intent == I_HELP)
		...
		update_icons()
	else if(istype(W, /obj/item/device/mmi) && opened )
		if(!mmi)
			var/obj/item/device/mmi/Wmmi = W
			if(Wmmi.brainmob)
				if(Wmmi.brainmob.mind)
					Wmmi.brainmob.mind.transfer_to(src)
				user.drop_item()
				W.loc = src
				to_chat(user, "You insert the MMI.")
				to_chat(src, "You take control of [src].")
			else
				to_chat(user, "This MMI lacks a functioning mind.")
		else
			to_chat(user, "There is already an MMI in this robot.")
