...
/mob/living/simple_animal/hostile/asteroid/vindictagolem
	name = "vindicta golem"
	desc = "A viscious looking creature with an orichilum exterior plating. It emits a huge amount of heat and is molten to the touch. "
	icon = 'icons/mob/mob.dmi'
	icon_state = "nothing"
	icon_living = "golem"
	icon_dead = "nothing"
	icon_gib = "nothing"
	icon_attacking = "golem-attacking"
	environment_smash = 1
	vision_range = 7
	aggro_vision_range = 9
	idle_vision_range = 7
	move_to_delay = 5
	friendly = "harmlessly brushes against"
	maxHealth = 60
	health = 60
...
/mob/living/simple_animal/hostile/asteroid/vindictagolem/New()
	..()
	zone_sel = pick("head", "chest", "groin", "l_arm", "l_hand", "r_arm", "r_hand", "l_leg", "l_foot", "r_leg", "r_foot")
	dir = pick(cardinal)
	flick("golem-entry", src)
	spawn(10)
		icon_state = "golem"
		AIStatus = AI_ON
/mob/living/simple_animal/hostile/asteroid/vindictagolem/GiveTarget(var/new_target)
	target = new_target
	if(target != null)
		visible_message("<span class='notice'>The [src.name] glares at [target] with a  burning rage.</span>")
...
/////////
/////////PETRA SPIDER
/mob/living/simple_animal/hostile/asteroid/petraspider
	name = "petra-spider"
	desc = "A small arachnid with razor sharp legs, fangs and claws."
	icon = 'icons/mob/animal.dmi'
	icon_state = "petraspider"
	icon_living = "petraspider"
	icon_dead = "petraspider-dead"
	icon_gib = "syndicate_gib"
	icon_attacking = "petraspider-attacking"
	icon_movement = "petraspider-moving"
	environment_smash = 0
	vision_range = 4
	aggro_vision_range = 9
	idle_vision_range = 4
	...
	search_objects = 1
	wanted_objects = list(/obj/structure/resourcepipe, /obj/machinery/auger, /obj/structure/ore_box, /obj/structure/pressure_tank, /obj/structure/plasma_canister)
	var/alert_status = 0
	var/next_eat = 0
	uses_autoemote = 1
...
/mob/living/simple_animal/hostile/asteroid/petraspider/New()
	..()
	zone_sel = pick("head", "chest", "groin", "l_arm", "l_hand", "r_arm", "r_hand", "l_leg", "l_foot", "r_leg", "r_foot")
/mob/living/simple_animal/hostile/asteroid/petraspider/PickTarget(var/list/Targets)//Step 3, pick amongst the possible, attackable targets
...
/mob/living/simple_animal/hostile/asteroid/petraspider/CanAttack(var/atom/target)//Can we actually attack a possible target?
	if(istype(target, /obj))
		if(next_eat <= world.time)
			if(istype(target, /obj/machinery/auger))
				var/obj/machinery/auger/aug = target
				if(aug.broken) return 0
			return ..()
		else
			return 0
		...
		if(alert_status)
			...
			if(M.pulling)
				if(istype(M.pulling, /obj/structure/ore_box) || istype(M.pulling, /obj/structure/pressure_tank) || istype(M.pulling, /obj/structure/plasma_canister) || istype(M.pulling, /obj/machinery/auger))
					if(..())
						visible_message("<span class='warning'>The [src.name] thinks [target] is trying to steal food its food!</span>")
						Aggro()
						return 1
			return 0
			
/mob/living/simple_animal/hostile/asteroid/petraspider/GiveTarget(var/new_target)
	target = new_target
	if(target != null)
		if(istype(target, /obj))
			if(next_eat <= world.time)
				visible_message("<span class='notice'>The [src.name] looks at [target] with hungry eyes.</span>")
		else if(isliving(target))
			visible_message("<span class='notice'>The [src.name] hisses at [target] angrily.</span>")
				
/mob/living/simple_animal/hostile/asteroid/petraspider/adjustHealth(damage)
	..(damage)
	if(damage)
		if(!stat && prob(damage))
			new /obj/effect/decal/cleanable/blood/xeno(loc)
			
	if(!stat && !alert_status)//Not unconscious, and its the first hit
		Aggro()
		for(var/mob/living/simple_animal/hostile/asteroid/petraspider/spid in orange(7, loc))
			if(!spid.alert_status)
				spid.Aggro()
/mob/living/simple_animal/hostile/asteroid/petraspider/AttackingTarget()
	var/ob = 0
	if(istype(target, /obj/))
		ob = 1
		visible_message("<span class='warning'>[src] takes a bite out of the [target.name]!</span>")
		playsound(src.loc,'sound/weapons/bite.ogg', rand(10,50), 1)
	if(istype(target, /mob/))
		attacktext = pick("rears its front razor-legs and jumps at", "swings its front legs wildly at", "uses its front legs to slice into")
	var/t = ..()
	if(!target && ob) // target was deeestroyed
		visible_message("<span class='notice'>[src] desperately bites at the metal scraps, temporarily sated!</span>")
		next_eat = (world.time + 30 SECONDS)
	return t
...
/mob/living/simple_animal/hostile/asteroid
	...
	var/icon_movement = null // for flick() when we move.
	var/icon_attacking = null
	see_in_dark = 8
	see_invisible = SEE_INVISIBLE_MINIMUM
	var/uses_autoemote = 0
	var/list/autoemotes = list()
	var/list/autosounds = list()
/mob/living/simple_animal/hostile/asteroid/AttackingTarget()
	dir = get_dir(src, target)
	if(icon_attacking)
		flick(icon_attacking, src)
	..()

/mob/living/simple_animal/hostile/asteroid/Life()
	. = ..()
	if(!stat && uses_autoemote && autoemotes.len)
		if(last_autoemote < world.time)
			visible_message("The [src] [pick(autoemotes)]")
			last_autoemote = ( world.time + rand(250, 600))
			if(autosounds.len)
				playsound(src.loc, pick(autosounds), rand(50,100), 1)
/mob/living/simple_animal/hostile/asteroid/Aggro()
	..()
	if(icon_aggro)
		icon_state = icon_aggro
/mob/living/simple_animal/hostile/asteroid/Move()
	if(!stat && icon_movement)
		flick(icon_movement,src)
	..()	
