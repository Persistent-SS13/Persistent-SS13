...
// PERSISTANT EDIT! RAN ZONE PICKS EITHER THE ORGAN OR ONE CLOSE TO IT
/proc/ran_zone(var/obj/item/organ/external/zone, probability = 80)
	if(!istype(zone))
		return ran_zone_old(zone, probability)
	if(prob(probability))
		return zone
	var/list/le = list()
	if(zone.children)
...
/proc/ran_zone_old(zone, probability = 80)
