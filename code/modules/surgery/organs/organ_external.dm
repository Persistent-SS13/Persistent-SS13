...
/obj/item/organ/external
	...
	map_storage_saved_vars = "density;icon_state;dir;name;pixel_x;pixel_y;damage;species;perma_injury;open;stage;cavity;sabotaged;encased;hidden;implants;wounds;children;parent;dna;species;internal_organs"
...
/****************************************************
			   DAMAGE PROCS
****************************************************/
/obj/item/organ/external/stump/take_damage(brute, burn, sharp, edge, used_weapon = null, list/forbidden_limbs = list())
	if(parent)
		return parent.take_damage(brute, burn, sharp, edge, used_weapon, forbidden_limbs)
	. = ..()
...
#define LIMB_DMG_PROB 5
	...
	if(used_weapon)
		...
		// PERSISTANT EDIT HERE! IF IT AINT SHARP, IT SHWONT CUT!
	var/can_cut = sharp && !(status & ORGAN_ROBOT)
	...
	if(owner && loc == owner)
		if(!cannot_amputate && config.limbs_can_break && (brute_dam) >= (max(max_damage * config.organ_health_multiplier, max_damage))) // the limb should possibly tear off before it starts distributing damage
			if(edge)
				if(prob(max(brute * 2, 100)))
					if(sharp)
						droplimb(0,DROPLIMB_EDGE)
					else
						droplimb(0,DROPLIMB_BLUNT)
			else
				if(prob(max(brute / 4, 100)))	
					if(sharp)
						droplimb(0,DROPLIMB_EDGE)
					else
						droplimb(0,DROPLIMB_BLUNT)
			if(prob(max(burn * 2, 100)))
...
/obj/item/organ/external/proc/createwound(var/type = CUT, var/damage)
	...
	if(wounds.len > 0)
		if((type == CUT || type == BRUISE || type == BURN))
			//we need to make sure that the wound we are going to worsen is compatible with the type of damage...
			var/list/compatible_wounds = list()
			for(var/datum/wound/W in wounds)
				if(W.can_worsen_new(type, damage))
					compatible_wounds += W

			if(compatible_wounds.len)
				var/datum/wound/W = pick(compatible_wounds)
				var/datum/wound/Wo = W.open_wound(damage)
				if(Wo)
					if(status & ORGAN_ROBOT)
						...
						owner.visible_message("<span class='alert'>[Wo.opening_desc_audible] The wound on [owner.name]'s [name] worsens into a [Wo.desc].</span>",\
						"<span class='alert'>On your [name], [Wo.opening_desc_visible]</span>",\
						"[Wo.opening_desc_audible]")
...
/obj/item/organ/external/proc/createwoundold(var/type = CUT, var/damage) // depreciated old version
	if(damage == 0) return
...
/obj/item/organ/external/proc/update_wounds()
	...
	for(var/datum/wound/W in wounds)
		...
		if(W.can_autoheal())
			...
			if(W.is_treated())
				heal_amt += 1
			if(W.surgery_treated) // big wounds can be treated through surgery and they heal twice as fast
				heal_amt += 1
		...
		// amount of healing is spread over all the wounds DEPRECIATED! ALL WOUNDS DESERVE CHANCE TO HEAL
	//	heal_amt = heal_amt / (wounds.len + 1)
		// making it look prettier on scanners
	//	heal_amt = round(heal_amt,0.1)
		W.heal_damage(heal_amt, 1, 1)

		// Salving also helps against infection 
		if(W.germ_level > 0 && (W.salved || W.bandaged) && prob(2))
			W.disinfected = 1
			W.germ_level = 0
...
/obj/item/organ/external/proc/droplimb(var/clean, var/disintegrate, var/ignore_children, var/nodamage)
	...
	switch(disintegrate)
		...
		if(DROPLIMB_BLUNT)
			...
			qdel(src) // If you flashed away to ashes, YOU FLASHED AWAY TO ASHES PERSISTANT THIS WILL NEED TO BE FIXED!
			return
...
/obj/item/organ/external/proc/bandage(var/testing = 0)
	var/rval = 0
...
/obj/item/organ/external/proc/splint(var/testing = 0)
	var/rval = 0
	for(var/datum/wound/W in wounds)
		if(W.internal) continue
		...
		rval |= !W.splinted
		if(!testing)	// sometimes this proc is just used to deterimine if theres a need for a splint
			W.splinted = 1
	return rval
	
/obj/item/organ/external/proc/get_int_organ(typepath)
...
/obj/item/organ/external/proc/salve(var/testing = 0)
	var/rval = 0
	for(var/datum/wound/W in wounds)
		if(W.internal) continue
...
/obj/item/organ/external/proc/fracture() // DEPRECIATED
