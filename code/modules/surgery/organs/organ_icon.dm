...
/obj/item/organ/external/proc/sync_colour_to_dna()
	...
	if(!isnull(dna.GetUIValue(DNA_UI_SKIN_TONE)) && ((species.bodyflags & HAS_SKIN_TONE) || (species.bodyflags & HAS_ICON_SKIN_TONE)))
		...
		s_col = null
	if(species.flags & HAS_SKIN_COLOR && !s_tone)
		s_col = list(dna.GetUIValueRange(DNA_UI_SKIN_R, 255), dna.GetUIValueRange(DNA_UI_SKIN_G, 255), dna.GetUIValueRange(DNA_UI_SKIN_B, 255))
		s_tone = null
...
/obj/item/organ/external/head/get_icon()
	...
	if(species.has_organ["eyes"])
		var/obj/item/organ/internal/eyes/eyes = src.get_int_organ(/obj/item/organ/internal/eyes)//owner.internal_organs_by_name["eyes"]
		if(species.eyes)
	...
	if(owner && owner.lip_style && (species && (species.flags & HAS_LIPS)))
		var/icon/lip_icon = new/icon('icons/mob/human_face.dmi', "lips_[owner.lip_style]_s")
		overlays |= lip_icon
		mob_icon.Blend(lip_icon, ICON_OVERLAY)

	if(owner && owner.m_style)
	...
	if(H.f_style)
		...
		if(facial_hair_style && ((facial_hair_style.species_allowed && (H.species.name in facial_hair_style.species_allowed)) || (src.species.flags & ALL_RPARTS)))
			...
			if(H.species.name == "Slime People" && owner) // I am el worstos
				facial_s.Blend(rgb(owner.r_skin, owner.g_skin, owner.b_skin, 160), ICON_AND)
			else if(facial_hair_style.do_colouration)
				facial_s.Blend(rgb(H.r_facial, H.g_facial, H.b_facial), ICON_ADD)
			overlays |= facial_s

	if(H.h_style) //  && owner && !(owner.head && (owner.head.flags & BLOCKHEADHAIR))
